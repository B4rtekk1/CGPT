...
bpe::TokenId sample(const std::vector<float>& logits, const GenerationOptions& options,
                    std::mt19937_64& rng) {
    ...
    for (std::size_t i = 0; i < logits.size(); ++i) {
        if (std::isfinite(logits[i])) candidates.push_back({static_cast<bpe::TokenId>(i), logits[i]});
    }
    if (candidates.empty()) throw std::runtime_error("all model logits are non-finite");
...
std::vector<bpe::TokenId> generate_tokens(
    ...
    if (prompt_tokens.size() > options.max_context_tokens) {
        throw std::invalid_argument("prompt exceeds max_context_tokens");
    }
...
std::string generate_text(
    ...
    if (!text_options.eos_token_id) {
        text_options.eos_token_id = tokenizer.special_token_id("<eos>");
    }
