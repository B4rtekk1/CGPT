...
Arguments parse_arguments(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (option == "--device") { if (++i >= argc) usage("missing device"); result.device = argv[i]; if (result.device != "auto" && result.device != "cpu" && result.device != "cuda") usage("device must be auto, cpu or cuda"); }
...
struct LayerStorage {
    ...
    explicit LayerStorage(const TransformerBlockOptions& b, DeviceType device)
        : attention_norm({b.hidden_size}, Dtype::F16, device), q({b.num_query_heads * b.head_dim, b.hidden_size}, Dtype::F16, device),
          k({b.num_kv_heads * b.head_dim, b.hidden_size}, Dtype::F16, device), q_norm({b.head_dim}, Dtype::F16, device),
          k_norm({b.head_dim}, Dtype::F16, device), v({b.num_kv_heads * b.head_dim, b.hidden_size}, Dtype::F16, device),
          o({b.hidden_size, b.num_query_heads * b.head_dim}, Dtype::F16, device), ffn_norm({b.hidden_size}, Dtype::F16, device),
          gate({b.intermediate_size, b.hidden_size}, Dtype::F16, device), up({b.intermediate_size, b.hidden_size}, Dtype::F16, device),
...
struct ModelStorage {
    ...
    explicit ModelStorage(TransformerModelOptions o, DeviceType device) : options(o),
        embedding({o.vocabulary_size, o.block_options.hidden_size}, Dtype::F16, device),
        final_norm({o.block_options.hidden_size}, Dtype::F16, device),
        lm_head({o.vocabulary_size, o.block_options.hidden_size}, Dtype::F16, device) {
...
TextGenerationResult generate(const std::string& prompt, const Arguments& args,
    ...
    if (args.cpu) return generate_text_with_stats_cpu(tokenizer, prompt, model.weights(), cosine, sine, model.options, options);
...
int main(int argc, char** argv) {
    try {
        ...
        for (std::string prompt; std::cout << "> " && std::getline(std::cin, prompt); ) {
            ...
            try { print_generation(prompt, runtime_args, tokenizer, model, cosine, sine, cublas.get()); }
